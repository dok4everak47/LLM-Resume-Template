...
\begin{itemize}
    ...
    \begin{itemize}
        \item 从零实现 Claude Code 核心 Agent Harness 架构: Agent Loop、13 个内置工具、流式并行执行、分层上下文压缩与多类型记忆系统、MCP 协议集成（总代码量约 5000 行）
        \item \textbf{实现 Sub-Agent 分治架构（fork-return）}: 主 Agent 派生 explore / plan / general 子 Agent，借只读权限隔离保护主会话上下文，支撑复杂任务自动拆解
        \item 构建 Multi-Agent 可视化演练场（React + react-three-fiber 3D）: 实时呈现子 Agent 委派 / 进度 / 结果流，内置代码审查、研究报告、客服 3 套协作场景，已 Vercel 部署上线
    \end{itemize}
    ...
    以中国大陆四城（北京 / 上海 / 广州 / 深圳）25 条线路、684 座车站的中英文站名为题的打字练习网站；采用完全数据驱动架构，城市与线路存于单一 \texttt{metro.json}，新增城市 / 线路无需改动应用逻辑。
    ...
    \begin{itemize}
        \item 用 Python 脚本从公开资料生成结构化站点 / 线路数据（含经纬度、中英文站名、拼音），覆盖环线（北京 2 / 10 号、上海 4 号）与支线的真实服务区段
        \item 借助 d3-geo 的 geoMercator 投影依真实站序与经纬度绘制路线；手工实现投影变换与 viewBox 自动取景，规避 d3 fitExtent 在多边形上的缩放缺陷，保证线路居中且不变形
        \item 首页全国省份底图（DataV.GeoAtlas）+ 城市标记，经两级导航（城市概览 → 线路详情 → 打字）进入游戏
        \item 完整中文输入法支持：基于 compositionstart / update / end 事件处理 IME，选字过程中不误判、不中断，并对简繁变体做归一化比对
        \item 打字反馈：中英文双模式、WPM / CPM、正确率、已到站数、列车沿线路实时推进动画，错误时抖动提示；含限时 / 沿线两种模式与正反向、多区段切换
        \item 深色模式、全键盘操作（含 ESC 返回）、响应式布局与 \texttt{prefers-reduced-motion} 无障碍适配
    \end{itemize}
\end{itemize}
